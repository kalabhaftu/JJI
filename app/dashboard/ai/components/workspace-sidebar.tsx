import { Archive, Eye, History, PanelLeftClose, Pin, Plus, Trash2, X } from 'lucide-react'
              ...
              <div className="space-y-1">
                {chats.filter((chat) => !chat.isArchived).map((chat) => (
                  <div key={chat.id} className={cn('group flex items-center rounded-xl', selectedChatId === chat.id ? 'bg-muted' : 'hover:bg-muted/60')}>
                    ...
                    >
                      {chat.isPinned ? <Pin className="h-3.5 w-3.5 shrink-0" /> : <History className="h-3.5 w-3.5 shrink-0 text-muted-foreground" />}
                    ...
                    <div className="flex pr-1 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 lg:group-focus-within:opacity-100">
                      <IconAction label={chat.isPinned ? 'Unpin conversation' : 'Pin conversation'} onClick={() => onTogglePin(chat)}><Pin /></IconAction>
                      <IconAction label="Archive conversation" onClick={() => onToggleArchive(chat)}><Archive /></IconAction>
                      <IconAction label="Delete conversation" destructive onClick={() => onDeleteChat(chat.id)}><Trash2 /></IconAction>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {insights.length === 0 ? <EmptyState>No saved insights yet.</EmptyState> : insights.map((insight) => (
              <article key={insight.id} className="group py-3 first:pt-0">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <IconAction label="Delete insight" destructive onClick={() => onDeleteInsight(insight.id)}><X /></IconAction>
                </div>
