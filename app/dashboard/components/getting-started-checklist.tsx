    ...
    <section className="mb-4 rounded-xl border border-border bg-card p-4" aria-labelledby="getting-started-title">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="flex items-start gap-3">
          <div className="rounded-lg bg-primary/10 p-2 text-primary"><HugeiconsIcon icon={Compass01Icon} className="h-4 w-4" strokeWidth={2} color="currentColor" /></div>
        ...
        <div className="flex items-center gap-1">
          <Button asChild variant="tertiary" size="sm"><Link href="/dashboard/settings?tab=help">View all tours <HugeiconsIcon icon={ChevronRightIcon} className="ml-1 h-4 w-4" strokeWidth={2} color="currentColor" /></Link></Button>
          <Button variant="icon-only" size="icon" className="h-11 w-11" onClick={dismiss} aria-label="Dismiss getting started checklist"><HugeiconsIcon icon={Cancel01Icon} className="h-4 w-4" strokeWidth={2} color="currentColor" /></Button>
        </div>
      </div>
      <div className="mt-4 flex flex-wrap gap-2">
        {ITEMS.slice(0, 5).map(({ id, label }) => {
          const isComplete = onboardingStatus.tours[id]?.state === 'completed'
          return <Button key={id} variant={isComplete ? "secondary" : id === next?.id ? "primary" : "tertiary"} size="sm" className="gap-2" onClick={() => startTour(id)}><HugeiconsIcon icon={Tick01Icon} className={isComplete ? 'h-3.5 w-3.5' : 'h-3.5 w-3.5 opacity-30'} strokeWidth={2} color="currentColor" />{label}</Button>
        })}
      </div>
    </section>
  )
