...
export function SettingsIntegrations({ token, loading, copied, regenerating, onCopyUrl, onRegenerate }: WebhookProps) {
  return <section className="flex flex-col gap-6" aria-labelledby="settings-integrations-heading">
    ...
    <div className="flex flex-col gap-6" data-tour="settings-card-integrations">
      <div className="flex items-center gap-3"><div className="flex size-10 shrink-0 items-center justify-center rounded-lg bg-muted"><HugeiconsIcon icon={WebhookIcon} className="size-5 text-muted-foreground" strokeWidth={2} color="currentColor" /></div><div><h3 className="text-sm font-semibold text-heading-text">TradingView Webhook</h3><p className="text-xs text-muted-foreground">Auto-import trades via TradingView alerts</p></div></div>
...
type SecurityProps = { editingProfile: boolean; onSignOutPrompt: () => void; onSignOutAll: () => void; onDelete: () => void }
export function SettingsSecurity({ editingProfile, onSignOutPrompt, onSignOutAll, onDelete }: SecurityProps) {
  return <section className="flex flex-col gap-6" aria-labelledby="settings-security-heading"><header><h2 id="settings-security-heading" className="text-lg font-semibold text-heading-text">Security &amp; Data Management</h2><p className="text-xs text-muted-foreground/85">Manage your local storage cache, export data, sign out, or delete your account</p></header><div data-tour="settings-card-security"><CacheManagement plain /></div><section className="flex flex-col gap-6"><h3 className="flex items-center gap-2 text-sm font-semibold text-heading-text"><HugeiconsIcon icon={Shield01Icon} className="size-4" strokeWidth={2} color="currentColor" />Account Actions</h3><div className="flex flex-wrap gap-3"><Button asChild variant="secondary" size="sm"><Link href="/dashboard/data"><HugeiconsIcon icon={DatabaseIcon} className="size-4" strokeWidth={2} color="currentColor" />Data Management</Link></Button><Button variant="secondary" size="sm" className="gap-2" onClick={onSignOutPrompt}><HugeiconsIcon icon={Logout01Icon} className="size-4" strokeWidth={2} color="currentColor" />Sign Out</Button><Button variant="secondary" size="sm" className="gap-2" onClick={onSignOutAll}><HugeiconsIcon icon={Logout01Icon} className="size-4" strokeWidth={2} color="currentColor" />Sign out all devices</Button></div><Separator /><SettingsDangerZone><p className="text-xs text-muted-foreground/85 text-balance">Permanently delete your account and all associated trading data, files, and settings. This action is irreversible.</p><Button variant="secondary" size="sm" className="w-fit gap-2 border-destructive/30 hover:border-destructive hover:bg-destructive/10" onClick={onDelete}><HugeiconsIcon icon={Delete02Icon} className="size-4" strokeWidth={2} color="currentColor" />Delete Account</Button></SettingsDangerZone></section></section>
}
