...
export async function syncWhopPayment(
  ...
  const paymentRecord = await db.transaction(async (tx) => {
    const [stored] = await tx.insert(PaymentRecord).values({
      ...
      providerStatus,
      whopMembershipId: membershipResult.storedMembership.membershipId,
      whopUserId: membershipResult.storedMembership.whopUserId,
      whopPlanId: membershipResult.storedMembership.planId,
      whopProductId: membershipResult.storedMembership.productId,
      whopEnvironment: membershipResult.storedMembership.environment,
