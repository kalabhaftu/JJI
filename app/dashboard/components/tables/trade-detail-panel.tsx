        ...
        <div className="px-4 sm:px-6 py-3 border-b border-border/40 shrink-0">
          <div className="flex items-center justify-between gap-3">
            ...
            <Button variant="secondary" onClick={handleEdit} className="h-8 px-3 rounded-lg text-xs font-semibold shrink-0">
              <HugeiconsIcon icon={PencilEdit01Icon} className="h-3.5 w-3.5 mr-1.5" strokeWidth={2} color="currentColor" />
              Edit
            </Button>
          </div>
        </div>
                    ...
                    <div className="flex items-center gap-1.5 group">
                      ...
                      {(field === 'entryPrice' || (field === 'closePrice' && value !== '-')) && (
                        <button
                          ...
                          title={`Copy ${label}`}
                        >
                          {copiedField === label ? <HugeiconsIcon icon={Tick01Icon} className="h-3 w-3 text-long" strokeWidth={2} color="currentColor" /> : <HugeiconsIcon icon={Copy01Icon} className="h-3 w-3 text-muted-foreground" strokeWidth={2} color="currentColor" />}
                        </button>
                      )}
                    </div>
                  </div>
              ...
              <div className="flex flex-wrap gap-x-6 gap-y-2 mt-4 pt-3 border-t border-border/20 text-xs text-muted-foreground">
                {tradeData.stopLoss && (
                  <span className="flex items-center gap-1">
                    ...
                    >
                      {copiedField === 'Stop Loss' ? <HugeiconsIcon icon={Tick01Icon} className="h-2.5 w-2.5 text-long" strokeWidth={2} color="currentColor" /> : <HugeiconsIcon icon={Copy01Icon} className="h-2.5 w-2.5" strokeWidth={2} color="currentColor" />}
                    </button>
                  </span>
                )}
                {tradeData.takeProfit && (
                  <span className="flex items-center gap-1">
                    ...
                    <button
                      ...
                      className="p-1 hover:bg-accent rounded-md"
                    >
                      {copiedField === 'Take Profit' ? <HugeiconsIcon icon={Tick01Icon} className="h-2.5 w-2.5 text-long" strokeWidth={2} color="currentColor" /> : <HugeiconsIcon icon={Copy01Icon} className="h-2.5 w-2.5" strokeWidth={2} color="currentColor" />}
                    </button>
                  </span>
                )}
                    ...
                    <div className="flex flex-wrap gap-2">
                      {chartLinks.map((link: string, index: number) => (
                        <a key={index} href={link} target="_blank" rel="noopener noreferrer"
                          ...
                          <HugeiconsIcon icon={BarChartIcon} className="h-3.5 w-3.5 text-muted-foreground" strokeWidth={2} color="currentColor" />
                          Chart {index + 1}
                        </a>
                      ))}
                    </div>
                        ...
                        <>
                          ...
                          {tradeData.newsTraded && (
                            <div className="flex items-center gap-2 px-2.5 py-1.5 rounded-lg bg-warning/10 border border-warning/20 text-warning">
                              <HugeiconsIcon icon={FlashIcon} className="h-3.5 w-3.5 fill-warning text-warning" strokeWidth={2} color="currentColor" />
        ...
        <div className="px-4 sm:px-6 py-3 border-t border-border/40 bg-muted/5 shrink-0 flex flex-col-reverse sm:flex-row items-center justify-between gap-2">
          <Link href={`/dashboard/table?view=replay&tradeId=${trade.id}&backUrl=${encodeURIComponent(typeof window !== 'undefined' ? window.location.pathname + window.location.search : basePath)}`} className="w-full sm:w-auto">
            <Button variant="primary" className="gap-2 h-9 px-5 rounded-xl shadow-lg shadow-primary/10 font-semibold w-full sm:w-auto text-xs">
              <HugeiconsIcon icon={PlayIcon} className="h-3.5 w-3.5" strokeWidth={2} color="currentColor" />
              Trade Replay
            </Button>
          </Link>
            ...
            <div className="relative flex-1 bg-black">
              ...
              {showImageNavigation && (
                <>
                  ...
                  >
                    <HugeiconsIcon icon={ChevronLeftIcon} className="h-5 w-5" strokeWidth={2} color="currentColor" />
                  </Button>
                  <Button
                    variant="secondary"
                    size="icon"
                  ...
                  >
                    <HugeiconsIcon icon={ChevronRightIcon} className="h-5 w-5" strokeWidth={2} color="currentColor" />
                  </Button>
