...
export async function generateJournalAnalysis(
  ...
  const { emotionCounts, emotionPerformance, journalSummary, tradeNotes, tradeStats } = analysisContext
  ...
  try {
    ...
    const providerAnalysis = await requestJournalAnalysis({ apiKey, baseUrl, model, prompt })
    return providerAnalysis ?? generateRuleBasedAnalysis(journalSummary, tradeStats, emotionCounts, emotionPerformance, tradeNotes)
  } catch (error) {
    reportError(error, { surface: 'server', operation: 'request-journal-analysis-provider', extra: { fallbackUsed: true } })
