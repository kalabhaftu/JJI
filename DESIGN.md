...
Accent preferences apply one of `accent-reports`, `accent-violet`, or `accent-slate`; Classic applies no accent class. Accent classes coexist with `light` or `dark` and override brand roles only. They must never override system status or financial meaning.
...
- Financial semantics: `--financial-profit`, `--financial-loss`, `--financial-long`, `--financial-short`, `--financial-bullish`, `--financial-bearish`, and `--financial-neutral` are stable within each light or dark theme. Profit/loss describe results, long/short describe direction, and bullish/bearish describe market movement. These roles are not interchangeable with system success or destructive actions.
