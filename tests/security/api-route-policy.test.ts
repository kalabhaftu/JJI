...
describe('API route policy registry', () => {
  it('classifies sensitive and trusted route families', () => {
    ...
    expect(classifyApiRoute('/api/v1/trades/import/jobs/123/process', 'POST')).toBe('import-job-process')
    expect(classifyApiRoute('/api/v1/data/import/jobs/456/process', 'POST')).toBe('import-job-process')
    expect(classifyApiRoute('/api/v1/trades/import/jobs/123', 'GET')).toBe('import-job-process')
