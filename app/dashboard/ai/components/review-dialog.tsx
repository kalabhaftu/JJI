      ...
      <DialogContent className="max-h-[88vh] overflow-y-auto sm:max-w-3xl">
        <DialogHeader>
          <div className="flex items-start justify-between gap-4 pr-8">
            <div>
              <DialogTitle className="flex items-center gap-2"><HugeiconsIcon icon={Brain01Icon} className="h-5 w-5" strokeWidth={2} color="currentColor" /> Weekly performance review</DialogTitle>
            ...
            {total > 1 && index !== null && (
              <div className="flex items-center gap-1 rounded-xl border border-border p-1">
                <Button variant="tertiary" size="icon" className="touch-target-compact h-8 w-8" disabled={index >= total - 1} onClick={() => onNavigate(index + 1)} aria-label="Older review"><HugeiconsIcon icon={ChevronLeftIcon} size={24} strokeWidth={2} color="currentColor" /></Button>
                <span className="min-w-12 text-center text-[11px] font-semibold text-muted-foreground">{index + 1} / {total}</span>
                <Button variant="tertiary" size="icon" className="touch-target-compact h-8 w-8" disabled={index <= 0} onClick={() => onNavigate(index - 1)} aria-label="Newer review"><HugeiconsIcon icon={ChevronRightIcon} size={24} strokeWidth={2} color="currentColor" /></Button>
              </div>
            )}
          </div>
        </DialogHeader>
        ...
        <div className="mt-4 space-y-7">
          ...
          <Button className="w-full" disabled={isSending} onClick={() => onDiscuss(review)}><HugeiconsIcon icon={Brain01Icon} size={24} strokeWidth={2} color="currentColor" /> Discuss this review</Button>
        </div>
      </DialogContent>
    </Dialog>
  )
