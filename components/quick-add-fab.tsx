...
export function QuickAddFAB({ className }: { className?: string }) {
  ...
  return <Dialog open={isDemo && isOpen} onOpenChange={setQuickAddOpen}>
    <Button type="button" size="icon" onClick={open} data-tour="quick-add-btn" className={cn('fixed bottom-28 right-6 z-[60] h-14 w-14 rounded-full shadow-md lg:hidden', className)} aria-label="Add trade"><HugeiconsIcon icon={Add01Icon} strokeWidth={1.5} color="currentColor" /></Button>
    <DialogContent><DialogHeader><DialogTitle>Quick Add Trade</DialogTitle><DialogDescription>Add a temporary trade to the demo.</DialogDescription></DialogHeader>
      <form className="flex flex-col gap-4" onSubmit={(event) => { event.preventDefault(); if (!instrument.trim() || !pnl.trim()) return; toast.success('Trade added (Demo mode: changes are temporary)'); setInstrument(''); setPnl(''); setSide('long'); setQuickAddOpen(false) }}>
        ...
        <div className="flex gap-2">
          <Button type="button" variant={side === 'long' ? "primary" : "secondary"} className="flex-1" onClick={() => setSide('long')}><HugeiconsIcon icon={ChartUpIcon} className="mr-2 h-4 w-4" strokeWidth={1.5} color="currentColor" />Long</Button>
          <Button type="button" variant={side === 'short' ? "primary" : "secondary"} className="flex-1" onClick={() => setSide('short')}><HugeiconsIcon icon={ChartDownIcon} className="mr-2 h-4 w-4" strokeWidth={1.5} color="currentColor" />Short</Button>
        </div>
