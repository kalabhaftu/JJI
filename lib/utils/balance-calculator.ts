...
// Prop firm phases share the same capital: count ONE starting balance per master
// account (prefer funded > active > passed) to avoid triple-counting.
export function calculateTotalStartingBalance(
  ...
  accounts.forEach(account => {
    // Prop firms keyed by master account ID; regular accounts by account ID
